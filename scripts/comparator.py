...
outputPath = "Documents/tests/pred_3_prednet_bot_c"
imageOutputPath = "Documents/imgs/pred_3_prednet_bot_c"
...
def newTest(TEST_SIZE):
    """
    @param TEST_SIZE: Number of tests needed
    ...
    a = {
        ...
        "2_cheb_pred": np.zeros(TEST_SIZE, bool),
        "4_cheb_pred": np.zeros(TEST_SIZE, bool),
        "6_cheb_pred": np.zeros(TEST_SIZE, bool),
        "2_cheb_m": np.empty(TEST_SIZE, dtype='<U50'),
        "4_cheb_m": np.empty(TEST_SIZE, dtype='<U50'),
        "6_cheb_m": np.empty(TEST_SIZE, dtype='<U50'),
...
class Comparator(object):
    ...
    def synchronize(self, msg):
        ...
        if self.status == "end":
            ...
            cv2.imwrite(imageOutputPath + self.name + ".png", image)
            self.test_counter += 1
            self.rec_counter = 0
    ...
    try:
        ...
        df = pd.DataFrame(data=ic.test,
                          columns=['name', 'start_time', "2_ed_pred", "2_ed_m", "4_ed_pred", "4_ed_m", "6_ed_pred",
                                   ...
                                   "6_cos_m"])
        df.to_csv(outputPath + uid + '.csv', sep=',')
